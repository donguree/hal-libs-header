...
typedef enum {
    ...
    HAL_PNG_PIXEL_FORMAT_YUV444SP,  /**< sequential packed with non-planar (YUVYUVYUV...) */
 ...
 *   .. code-block:: cpp
 *
 *     DTV_STATUS_T HAL_PNG_Open(void) {
 *       Open HW PNG resources
 *       If any eror THEN
 ...
 *   .. code-block:: cpp
 *
 *     DTV_STATUS_T HAL_PNG_Decode(src, dst, option) {
 ...
 *   .. code-block:: cpp
 *
 *     HAL_PNG_IMAGE_T src, dst;
 *     HAL_PNG_DECODE_OPTION_T option;
 *     src.buf = ...;
 *     src.len_buf = ...;
 *     src.rect = ...;
 *     src.image_format = ...;
 *     src.pixel_format = ...;
 *     option.scale = HAL_PNG_DECODE_OPTION_SCALING_1_1;
 *     ret = HAL_PNG_Decode (&src, &dst, option);
 *     if ( ret != API_OK )
 *       printf ("fail to decode the image");
 *
 * @endrst
 **/
DTV_STATUS_T HAL_PNG_Decode(HAL_PNG_IMAGE_T *src, HAL_PNG_IMAGE_T *dst, HAL_PNG_DECODE_OPTION_T *option);
