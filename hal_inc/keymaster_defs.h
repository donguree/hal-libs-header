...
/******************************************************************************
    Header File Guarder
******************************************************************************/
#ifndef _KEYMASTER_DEF_H_
#define _KEYMASTER_DEF_H_
...
typedef enum {
    ...
    KM_ERROR_ATTEST_ACCESS_CONFLICT = -110,
#endif
	KM_ERROR_INCOMPATIBLE_WRAP_DATA_TYPE = -200,
	KM_ERROR_TRANSFER_WRAP_DATA = -201,

    KM_ERROR_UNKNOWN_ERROR = -1000,
} keymaster_error_t;

/**
 * Data type for import wrapped data.
 */
typedef enum {
	KM_DATA_TYPE_NETFLIX	= 0,	/* for netflix data */
	KM_DATA_TYPE_PLAYREADY	= 1,	/* for playready data */
	KM_DATA_TYPE_AMAZON 	= 2,	/* for amazon data */
	KM_DATA_TYPE_DISNEY 	= 3,	/* for disney data */
} keymaster_data_type_t;
...
inline keymaster_key_param_t keymaster_param_enum(keymaster_tag_t tag, uint32_t value) {
    ...
    param.tag = tag;
    param.key_param.enumerated = value;
    return param;
}
...
inline keymaster_key_param_t keymaster_param_long(keymaster_tag_t tag, uint64_t value) {
    ...
    memset(&param, 0, sizeof(param));
    param.tag = tag;
    param.key_param.long_integer = value;
    return param;
}
...
inline keymaster_key_param_t keymaster_param_blob(keymaster_tag_t tag, const uint8_t* bytes,
    ...
    memset(&param, 0, sizeof(param));
    param.tag = tag;
    param.key_param.blob.data = (uint8_t*)bytes;
    param.key_param.blob.data_length = bytes_len;
    return param;
}
...
inline keymaster_key_param_t keymaster_param_date(keymaster_tag_t tag, uint64_t value) {
    ...
    memset(&param, 0, sizeof(param));
    param.tag = tag;
    param.key_param.date_time = value;
    return param;
}
...
inline int keymaster_param_compare(const keymaster_key_param_t* a, const keymaster_key_param_t* b) {
    ...
    case KM_ENUM:
    case KM_ENUM_REP:
        return KEYMASTER_SIMPLE_COMPARE(a->key_param.enumerated, b->key_param.enumerated);
    case KM_UINT:
    case KM_UINT_REP:
        return KEYMASTER_SIMPLE_COMPARE(a->key_param.integer, b->key_param.integer);
    case KM_ULONG:
    case KM_ULONG_REP:
        return KEYMASTER_SIMPLE_COMPARE(a->key_param.long_integer, b->key_param.long_integer);
    case KM_DATE:
        return KEYMASTER_SIMPLE_COMPARE(a->key_param.date_time, b->key_param.date_time);
    case KM_BIGNUM:
    case KM_BYTES:
        // Handle the empty cases.
        if (a->key_param.blob.data_length != 0 && b->key_param.blob.data_length == 0)
            return -1;
        if (a->key_param.blob.data_length == 0 && b->key_param.blob.data_length == 0)
            return 0;
        if (a->key_param.blob.data_length == 0 && b->key_param.blob.data_length > 0)
        ...
        retval = memcmp(a->key_param.blob.data, b->key_param.blob.data, a->key_param.blob.data_length < b->key_param.blob.data_length
                                                        ? a->key_param.blob.data_length
                                                        : b->key_param.blob.data_length);
        if (retval != 0)
            return retval;
        else if (a->key_param.blob.data_length != b->key_param.blob.data_length) {
            // Equal up to the common length; longer one is larger.
            if (a->key_param.blob.data_length < b->key_param.blob.data_length)
                return -1;
            if (a->key_param.blob.data_length > b->key_param.blob.data_length)
                return 1;
        };
...
inline void keymaster_free_param_values(keymaster_key_param_t* param, size_t param_count) {
    while (param_count > 0) {
        ...
        case KM_BIGNUM:
        case KM_BYTES:
            free((void*)param->key_param.blob.data);
            param->key_param.blob.data = NULL;
            break;
        default:
            // NOP
            break;
